...
read_gtex=function(tissue_id){
	gtex_dir='/mnt/lab_data/montgomery/shared/datasets/gtex/GTEx_Analysis_2016-01-15_v7/eqtls/all_associations/'
	suffix='.allpairs.txt.gz'
	gtex=fread(
		input = paste0("zcat ",gtex_dir,tissue_id,suffix," | sed 's/_/\t/g'"),
		sep = '\t',
		select = c(1,2,3,11),
...
# c("Adipose_Subcutaneous","Adipose_Visceral_Omentum","Adrenal_Gland","Artery_Aorta",
...
for (tissue_id in c("Heart_Left_Ventricle","Liver","Lung",
	...
	print(tissue_id)
	table_name = paste0('eQTL_',str_replace(tissue_id,'-','_'),'_GTEx_v7')
	...
	for (i in seq(1,total_row,step)) {
		...
		fwrite(gtex[start:end,list(trait = gene_id, rsid, pval)],'gtex_tmp.csv')

		dbExecute(
			conn = locuscompare_pool,
			statement = sprintf("load data local infile 'gtex_tmp.csv' 
			into table %s 
